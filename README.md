...
| 04 | `edge_vio_relay` | Server relocalization + kamera ARKit + **kamera browser** | Python + Swift + HTML/JS |
...
1. Deploy server (Modul 04) ke VPS - lihat `docs/deploy_to_vps.md`, atau
   ...
   pip install fastapi uvicorn python-multipart opencv-python-headless numpy --break-system-packages
...
3. Masukkan alamat server di kolom "Alamat Server OpenVPS" (default
   `http://localhost:8000` untuk tes lokal)
4. Klik "Aktifkan Kamera" -> izinkan akses kamera -> ambil 10-20 foto sambil
   berkeliling ruangan/objek (overlap 60-80% antar foto)
